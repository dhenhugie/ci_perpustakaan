...
FROM detail_pinjam D, peminjaman P, buku B, anggota A
WHERE B.id_buku=D.id_buku and
...
transaksi t,buku b,anggota a where b.id_buku=t.id_buku
and a.id_anggota=t.id_anggota and
a.id_anggota=$where")->result();
        $d = $this->M_perpus->edit_data(
            array('id_anggota' =>
            $this->session->userdata('id_agt')),
            'transaksi'
        )->num_rows();
    ...
    public function selesai_booking($where)
    {
        ...
        $data['items'] = $this->db->query("select * from
peminjaman p,detail_pinjam d, buku b where
b.id_buku=d.id_buku and d.id_pinjam=p.id_pinjam and
p.id_anggota='$where'")->result();
        $this->load->view('desain');
        $this->load->view(
            'toplayout',
            $data
