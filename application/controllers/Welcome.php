...
class Welcome extends CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() != false) {
			...
			$whereanggota = array('email' => $username, 'password' => md5($password), 'status' => '1');
			...
			if ($dataadmin->num_rows() > 0) {
				...
				$session = array('id' => $a->id_anggota, 'nama' => $a->nama_anggota, 'email' => $a->email, 'status' =>'login','role'=>'member');
				$this->session->set_userdata($session);
